 ...
 *
 *	12-apr-88 Daniel Lawrence
 *	Fixed a bug in the replaces() which would misplace the point after
 *	aborting a query-replace-string.
 */
 ...
 *	and (perhaps) repaint the display.
 */
int PASCAL NEAR mcscanner(mcpatrn, direct, beg_or_end)
 ...
 *	Software-Practice and Experience, vol 10, 501-506 (1980)
 */
int PASCAL NEAR scanner(patrn, direct, beg_or_end)
...
char apat[];
{
	...
	for (i = 0; i < patlenadd; i++)
	{
		 ...
		 */
		mlwrite(TEXT82, (unsigned int) apat[i], patlenadd - i);
/*                      "Considering %d with jump %d" */
		tgetc();
		if (isletter(apat[i]))
		{
			mlwrite(TEXT83, CHCASE((unsigned int) apat[i]));
/*                              "Its other case is %d" */
			tgetc();
		}
#endif
 ...
 */
PASCAL NEAR delins(dlength, instr, use_meta)
int	dlength;
char	*instr;
int	use_meta;
{
 ...
 */
PASCAL NEAR expandp(srcstr, deststr, maxlength)
