...
#define STACK_END_ADDR (0x80000000 - sizeof(int[5]))

/* Stack grows downward.  */

#define INNER_THAN <

/* On the VAX, sigtramp is at the top of the stack.   And that's it. */

#define IN_SIGTRAMP(pc, name)	((pc) >= STACK_END_ADDR)
...
#define MACH_ARCH_EXCEPTION_STRING	mach_vax_exception_string

/* Thread flavors used in re-setting the T bit */

#define TBIT_FLAVOR		VAX_THREAD_STATE
#define TBIT_FLAVOR_SIZE	VAX_THREAD_STATE_COUNT
#define TBIT_SET(x,state)	((struct vax_thread_state *)state)->ps =	\
				  ((struct vax_thread_state *)state)->ps | PSL_T
#define TBIT_CLEAR(x,state)	((struct vax_thread_state *)state)->ps =	\
				  ((struct vax_thread_state *)state)->ps & ~PSL_T
