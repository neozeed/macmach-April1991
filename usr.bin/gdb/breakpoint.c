...
insert_breakpoints ()
{
  ...
  ALL_BREAKPOINTS (b)
	...
	FLUSH_INFERIOR_CACHE(b->address, sizeof break_insn);
#endif
#ifdef BREAKPOINT_DEBUG
...
/* Get real content of memory at PC, where a breakpoint currently is.
   If no breakpoints or not inserted return CUR */
unsigned
breakpoint_shadow_val(pc,cur)
	CORE_ADDR pc;
	unsigned cur;
{
  register struct breakpoint *b;
  ...
  ALL_BREAKPOINTS (b)
    if (b->address == pc && b->inserted)
       return *(unsigned*)(b->shadow_contents);
  return cur;
}
...
#ifdef FLUSH_INFERIOR_CACHE
    FLUSH_INFERIOR_CACHE(bpt->address, sizeof break_insn);
#endif
  }
