...
/* Macro used to mark places where we know we leak memory.
   Flame: this really is GDB's fault.  If it was not so
   deeply convinced that it has to name types "struct foo"
   we would have no problem at all.  Instead, we have to
   prepend the "struct" to the typename.  Personally, I
   do not see any good reason for this: types live
   in a separate namespace anyways. Oh well. */
#define MAYBE_FREE(p)
...
/* Basically, this module must provide two functions: symbol_file_command()
   ...
   for real.  psymtab_to_symtab() is the function that does this */
	...
	char *name;
{
	 ...
	 */
	{
		...
		desc = openp(getenv("PATH"),
			     1, name, O_RDONLY, 0, &absolute_name);
		if (desc < 0)
	...
	char *name;
{
	...
	if (symtab_list &&
	    !query("Add more symbols from \"%s\"? ", name))
		error("Not confirmed.");
...
struct symtab *psymtab_to_symtab(pst)
	...
	int	*bs;
{
	...
	if (t->bt == btIndirect ||
		...
		if (TYPE_CODE(tp1) == TYPE_CODE_UNDEF) {
			...
			MAYBE_FREE(TYPE_NAME(tp1));	/* smash name as well */
			TYPE_NAME(tp1) = savestring(name, strlen(name));
	...
	PDR *pr;
{
	...
	static SYMR	static_procedure = { 0, 0, stStatic, scText, 0, 0};
	char name[100];
	...
	if (sh == (SYMR*)-1) {
		...
		sh->iss = (unsigned)savestring(name, strlen(name));
		goto new_proc;
	}
	s = mylookup_symbol(sh->iss, top_stack->cur_block, VAR_NAMESPACE, LOC_BLOCK);
	if (s != 0) {
	...
	HDRR *hdr;
{
	...
	for (s_idx = 0; s_idx < hdr->iextMax; s_idx++) {
		...
		case stLabel:
			...
			break;
		default:
			error("Unk external symbol %s %x %x\n",
	...
	int f_idx;
{
	...
	pst->textlow = fh->adr;
	/* Use texthigh for now to indicate whether this file contains
	   any code.  NOTE that minimally coded assemly might not
	   include any .ent statements, this causes cpd==0 but there
	   will be at least some global labels in it */	   
	pst->texthigh = fh->cpd;
	...
	struct partial_symtab *pst;
{
	...
	BLOCK_START(top_stack->cur_block) = pst->texthigh ? fh->adr : 0;
	BLOCK_END(top_stack->cur_block) = pst->texthigh;
...
/* Symbols are compared lexicographically by their print names */
...
/* Partial symbols are also compared lexicog by their print names */
...
/* Sort all symbols (in all blocks) of a symtab S */
...
fixup_sigtramp()
{
	...
	s = lookup_symbol("sigvec", 0, VAR_NAMESPACE, 0);
	...
	s = lookup_symbol("sigtramp", b0, VAR_NAMESPACE, 0);
