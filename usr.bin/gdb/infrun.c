...
#ifdef __GNUC__
static lookup_exception_actions();
#endif
...
#else	MACH
      /* Map machine exception into U*x signal */
      if (stopped_in_ptrace) 
	stop_signal = WSTOPSIG (w);
      else
	{
	  int print_exception, stop_for_exception, forward_exception;

	  if (stop_exception == EXC_BREAKPOINT)
	      stop_signal = SIGTRAP;
	  else
	    {
	      lookup_exception_actions (stop_exception, stop_code, 
					stop_subcode, &print_exception,
					&stop_for_exception,
					&forward_exception);
	      if (print_exception)
		{
		  terminal_ours_for_output ();
		  printf("\nProgram generated: %s.\n",
			 MACH_ARCH_EXCEPTION_STRING(stop_exception,
						    stop_code,
						    stop_subcode));
		  fflush(stdout);
		}
	      if (stop_for_exception)
		break;
	      if (print_exception)
		terminal_inferior ();
	      fatal("wait_for_inferior: what signal do I use?\n");
					/* "can't happen" */
	    }
	}
#endif	MACH
  ...
  if (step_resume_break_address && !step_resume_break_duplicate)
    {
      ...
      FLUSH_INFERIOR_CACHE(step_resume_break_address, sizeof break_insn);
#endif
    }
}
...
#ifdef	MACH
/* 
 * The user has the option of forwarding exceptions to another 
 * handler.  This table tells what to do.
 */
#define MAX_EXCEPTION	6		/* see <mach/exception.h> */
struct exception_list {
  char *name;
  boolean_t forward;
} exception_map[] = {
  {"not_an_exception",		FALSE},
  {"EXC_BAD_ACCESS",		FALSE},
  {"EXC_BAD_INSTRUCTION",	FALSE},
  {"EXC_BAD_ARITHMETIC",	FALSE},
  {"EXC_EMULATION",		FALSE},
  {"EXC_SOFTWARE",		FALSE},
  {"EXC_BREAKPOINT",		FALSE},
};

int max_exceptions = sizeof(exception_map)/sizeof(struct exception_list);


/* Decide whether to pass an exception on to another exception handler 
   or whether gdb should deal with it.
   Returns non-zero if the exception should be forwarded. */
/* I'm not sure how this function should relate to
   lookup_exception_actions.  I guess it depends on whether you think
   that the forwarding should be done in catch_exception raise or in
   wait_for_inferior.  */
...
fwd_exc_p (exception, code)
     int exception, code;
{
  if (exception > MAX_EXCEPTION || exception < 0)
    {
      printf ("Warning: unknown exception number (%d)\n", exception);
      return FALSE;
    }

  return exception_map[exception].forward;
}

static
lookup_exception_actions (exception, code, subcode, print, stop, forward)
     int exception, code, subcode;
     int *print, *stop, *forward;	/* yes/no */
{
  *forward = 0;
  *print = 1;
  *stop = 1;
...

/* 
 * Used for matching prefixes of commands and such.
 * XXX - should be in a more generally accessible place.
 */
#define Strprefix(s1,s2,len)	(!strncmp((s1), (s2), (len)))

/* Specify how various exceptions in the inferior should be handled.  */
     ...
     int from_tty;
{
  ...
  if (!args)
    error_no_arg ("exception to handle");
  ...
  while (*p)
    {
      ...
      if (digits == wordlen)
	excnum = atoi (p);
      else if (excnum > MAX_EXCEPTION || excnum <= 0)
	error ("Invalid exception number (must be in [1..%d]).",
	       MAX_EXCEPTION);

      /* Else, if already got an exception number, look for keyword
	 saying what to do for it.  */
      else if (Strprefix(p, "forward", wordlen))
	exception_map[excnum].forward = 1;
      else if (Strprefix(p, "keep", wordlen))
	exception_map[excnum].forward = FALSE;
      /* Not a number and not a recognized flag word => complain.  */
      else
	{
	  p[wordlen] = 0;
...
/* Print current contents of the tables set by the exception command.  */

static void
exception_info (excnum_exp)
     char *excnum_exp;
{
  register int i;
  static void print_one_exc();
  
  if (excnum_exp)
    {
      i = parse_and_eval_address (excnum_exp);
      print_one_exc (i);
      return;
    }
  ...
  printf_filtered ("\nUse the \"exception\" command to change this table.\n");
}
...
print_one_exc(exc)
     int exc;
{
  if (exc > MAX_EXCEPTION)
    printf_filtered ("No such exception (%d).\n", exc);
  else
    {
      printf_filtered ("%s (%d): ", exception_map[exc].name, exc);
      printf_filtered ("%s\n", (exception_map[exc].forward
				? "forward" : "keep"));
    }
}
#endif	MACH
...
handler.\n\
Stop means reenter debugger if this exception happens.\n\
Normally \"forward\" is used so that the inferior's signal handler\n\
can catch the signal that the exception maps to.");
