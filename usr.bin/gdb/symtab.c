...
#ifdef mac2
/* it seems that obstack.h exists in the gdb source directory */
#include "obstack.h"
#else
#include <obstack.h>
#endif
...
#ifdef	USE_ONLY_GLOBAL_PSYMS
  /* Shouldn't we check the globals too ? */
  for (p = global_psymbols.list + psymtab->globals_offset;
       (p - (global_psymbols.list + psymtab->globals_offset)
