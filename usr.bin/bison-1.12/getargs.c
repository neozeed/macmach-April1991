     ...
     char *argv[];
{
  ...
  while ((c = getopt_long (argc, argv, "yvdltVo:b:p:", longopts, &longind))
	...
	default:
	  fprintf (stderr, "\
Usage: %s [-dltvyV] [-a name-prefix] [-b file-prefix] [-o outfile]\n\
       [+defines] [+no-lines] [+debug] [+verbose] [+fixed-output-files]\n\
       [+yacc] [+name-prefix name-prefix] [+file-prefix file-prefix]\n\
       [+output-file outfile] [+version] grammar-file\n",
		   program_name);
	  exit (1);
	}
    }
