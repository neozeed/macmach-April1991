 ...
 *
 *  HISTORY
 * 20-Fev-91  Zon Williamson (zon) at Carnegie-Mellon University
 *	Added defaults for standard paths incase the user forgot.
 *
...
char *default_cpath = "/usr/include:/usr/local/include";
char *default_epath = 0;
char *default_lpath = "/lib:/usr/lib:/usr/local/lib";
char *default_mpath = "/usr/man:/usr/local/man";
char *default_path = "/etc:/bin:/usr/ucb:/usr/bin:/usr/old:/usr/bin/X11:/usr/local/bin:/usr/games";
 ...
 */
newpath(char *fpathname, char *default_pathlist)
{
  pathname = fpathname;
  if (!(path = getenv(fpathname)) && !(path = default_pathlist))
    ...
    while (option = *leadp++)
      {
        switch (option)
          ...
          case 'R': Rflag++; break;
          case 'X': Xflag++; break;
          case 'C': newpath("CPATH", default_cpath); break;
          case 'E': newpath("EPATH", default_epath); break;
          case 'L': newpath("LPATH", default_lpath); break;
          case 'M': newpath("MPATH", default_mpath); break;
          case 'P': newpath( "PATH", default_path); break;
