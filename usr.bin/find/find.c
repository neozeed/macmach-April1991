...
/* 9-Mar-88  Zon Williamson (zon) at Carnegie Mellon University
 *	Removed bogus file system assumption for mac2.
 */
...
struct s12 { int f; char *pat; };
glob(p)
register struct s12 *p; 
{
...
struct s11 { int f, t, s; };
mtime(p)
register struct s11 *p; 
{
	return(scomp((int)((Now - Statb.st_mtime) / A_DAY), p->t, p->s));
}
struct s10 { int f, t, s; };
atime(p)
register struct s10 *p; 
{
	return(scomp((int)((Now - Statb.st_atime) / A_DAY), p->t, p->s));
}
struct s9 { int f, u, s; };
user(p)
register struct s9 *p; 
{
...
struct s8 { int f, u, s; };
ino(p)
register struct s8 *p;
{
	return(scomp((int)Statb.st_ino, p->u, p->s));
}
struct s7 { int f, u; };
group(p)
register struct s7 *p; 
{
	...
	char *name, *fname;
{
	...
	(*exlist->F)(exlist);
#ifdef mac2
	/* if it's not a directory, return 1 */
	if((Statb.st_mode&S_IFMT)!=S_IFDIR) return(1);
	/* if it's a directory mounted on another device... */
	else if (Devstat.st_dev != Statb.st_dev) {
		/* if -xdev option, return 1 (don't traverse to different file system) */
		if (!Xdev) return(1);
	}
	/* if it's a directory in the same file system... */
	else {
		/* when nlink==2, ok to assume no subdirectories */
		/* this optimizes "find . -type d -print" */
