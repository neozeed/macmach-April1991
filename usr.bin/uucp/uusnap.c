#ifndef lint
static char sccsid[] = "@(#)uusnap.c	5.9	(Berkeley) 4/5/88";
#endif
...
scandirx(dnam, prfx, flen, fchr, type)
char *dnam, *prfx, fchr;
{
...
{	
	...
	scandirx(CMDSDIR, "C.", CMDSLEN, NULL, CMDTYPE);
	scandirx(DATADIR, "D.", DATALEN, NULL, DATTYPE);
	scandirx(XEQTDIR, "X.", XEQTLEN, 'X', XEQTYPE);
	getstst(SPOOL);
	time(&curtime);
	for(i=0; i<sndx; ++i)
