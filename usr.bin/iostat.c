 ...
 *      Needs read_name() for mac2, disk stats don't seem to work???.
 */
	...
	char *argv[];
{
	...
	if (read(mf, &kbase, sizeof(kbase)) != sizeof(kbase)) {
		perror("/dev/kmem");
		exit(1);
        }
#endif
	...
	lseek(mf, nl[X_DK_NDRIVE].n_value + kbase, L_SET);
...
#define	allocate(e, t) \
	...
	read_names();
	lseek(mf, (long)(nl[X_HZ].n_value + kbase), L_SET);
	read(mf, &hz, sizeof hz);
	lseek(mf, (long)(nl[X_PHZ].n_value + kbase), L_SET);
	read(mf, &phz, sizeof phz);
	if (phz)
		hz = phz;
	lseek(mf, (long)(nl[X_DK_MSPW].n_value + kbase), L_SET);
...
loop:
	if (--tohdr == 0)
		printhdr();
	lseek(mf, (long)(nl[X_DK_BUSY].n_value + kbase), L_SET);
 	read(mf, &s.dk_busy, sizeof s.dk_busy);
 	lseek(mf, (long)(nl[X_DK_TIME].n_value + kbase), L_SET);
 	read(mf, s.dk_time, dk_ndrive*sizeof (long));
 	lseek(mf, (long)(nl[X_DK_XFER].n_value + kbase), L_SET);
 	read(mf, s.dk_xfer, dk_ndrive*sizeof (long));
 	lseek(mf, (long)(nl[X_DK_WDS].n_value + kbase), L_SET);
 	read(mf, s.dk_wds, dk_ndrive*sizeof (long));
	lseek(mf, (long)(nl[X_DK_SEEK].n_value + kbase), L_SET);
	read(mf, s.dk_seek, dk_ndrive*sizeof (long));
 	lseek(mf, (long)(nl[X_TK_NIN].n_value + kbase), L_SET);
 	read(mf, &s.tk_nin, sizeof s.tk_nin);
 	lseek(mf, (long)(nl[X_TK_NOUT].n_value + kbase), L_SET);
 	read(mf, &s.tk_nout, sizeof s.tk_nout);
	lseek(mf, (long)(nl[X_CP_TIME].n_value + kbase), L_SET);
...
read_names()
{
  ...
  for (i = 0; i < 2; i++) sprintf(dr_name[i], "sd%d", i);
}
#endif
