...
input devices other than the system keyboard and pointer. 
...
data as well as other data that is not currently reportable via the core 
input events.
...
devices in just the same way as events from the X keyboard and X pointer.
...
.H 2 "Getting the Extension Version"
...
.br
.sp
The protocol version numbers returned indicate the protocol the 
input extension supports.
.H 2 "Listing Available Devices"
A client that wishes to access a specific device must first determine 
whether that device is connected to the X server.  This is done through the
\fBListInputDevices\fP request, which will return a list of all devices that
...
server,  
.mc >
including the X keyboard and X pointer.  Some implementations may open
...
.BL
.LI
The information returned for each device is:
.mc
.P
The \fBtype\fP field is of type Atom and indicates the nature of the device.  
A central
registry of device types will be established by the X Consortium and will
include:
.DS
\fBMOUSE\fP	
\fBTABLET\fP
...
may report several event classes.  For example, input class Keys reports
DeviceKeyPress and DeviceKeyRelease event classes.  Input classes are
unique numbers registered with the X Consortium.  Input class \fBOther\fP exists
...
.in -.5i
.sp 1.5
.H 2 "Changing The Mode Of A Device"
...
\fBRelative\fP.
.P 
This request will fail and return \fBBusy\fP if another client already
has the device open with a different mode.
.sp 1.5
SetDeviceMode
.in +.5i
device: DEVICE 
...
.br
.sp
Errors: Device, Match, Mode, DeviceBusy
.br
.in -.5i
.sp 1.5
=>
...
.in -.5i
.P
.H 2 "Selecting Extension Device Events"
Device input events are selected using the \fBSelectExtensionEvent\fP
request.
.sp 1.5
...
\fBButtonMotion1\fP through \fBButtonMotion5\fP, or \fBButtonMotion\fP.  
An input device will only support
as many button motion classes as it has buttons.
.H 2 "Determining Selected Events"
...
.H 2 "Controlling Event Propagation"
...
.in -.5i
.sp 1.5
.H 2 "Event Synchronization And Core Grabs"
...
This requests overrides any previous active grab by this client for this
device.
.P
...
the grabbing client issues a releasing \fBAllowEvents\fP request or until the 
...
active grab.
.H 2 "Passively Grabbing A Button"
...
ReplayThisdevice, AsyncAllDevices, or SyncAllDevices}
.br
time:TIMESTAMP or CurrentTime
.br
.sp
...
.H 2 "Controlling Device Feedback"
...
.br
.P
Each led in led_mask is set to the corresponding value in led_value. 
At most 32 LEDs are supported.  No standard interpretation of LEDs is defined.
.P
.mc >
...
FEEDBACKCONTROL:
T}	T{
{KBDFEEDBACKCONTROL, PTRFEEDBACKCONTROL, INTEGERFEEDBACKCONTROL, STRINGFEEDBACKCONTROL, BELLFEEDBACKCONTROL, LEDFEEDBACKCONTROL}
T}
.TE
.br
.P
...
.in -.5i
.P
Some devices contain leds.  This is done using
feedback class \fBLedFeedbackClass\fP using the \fBLedFeedbackControl\fP
structure.  The members of that structure are as follows:
.sp
.in +.5i
.TS
...
Errors:  Device, Match, Value
.P
.H 2 "Controlling Device Encoding"
...
and none of the modifiers are changed.  
.H 2 "Controlling Button Mapping"
...
not changed. 
.H 2 "Obtaining The State Of A Device"
...
.H 2 "Button, Key, and Motion Events"
.P
DeviceKeyPress
.br
DeviceKeyRelease
...
the buttons on the system pointer and modifier keys on the system keyboard
...
events.
.H 2 "Device KeyState and ButtonState Notify Events"
.P
