...
static long macmachEventTime();
static Bool macmachCursorOffScreen();
static void macmachCrossScreen();
extern void miPointerQueueEvent();

miPointerCursorFuncRec macmachPointerCursorFuncs = {
    macmachEventTime,
    macmachCursorOffScreen,
    macmachCrossScreen,
    miPointerQueueEvent,
};
...
} macmachMsPrivRec, *macmachMsPrivPtr;

static void 	  	macmachMouseCtrl();
static int 	  	macmachMouseGetMotionEvents();
void 	  		macmachMouseProcessEvent();
static void 	  	macmachMouseDoneEvents();

static macmachMsPrivRec	macmachMousePriv;
static PtrPrivRec 	sysMousePriv = {
    macmachMouseProcessEvent,	/* Function to process an event */
    macmachMouseDoneEvents,		/* When all the events have been */
				/* handled, this function will be */
				/* called. */
    0,				/* Current X coordinate of pointer */
    0,				/* Current Y coordinate */
    NULL,			/* Screen pointer is on */
    (pointer)&macmachMousePriv,	/* Field private to device */
};
...
*/
	    ...
	    macmachMousePriv.mouseMoved = FALSE;
		...
		pMouse, map, 3, macmachMouseGetMotionEvents, macmachMouseCtrl, 0);
	    break;
 ...
 */
static int
macmachMouseGetMotionEvents (buff, start, stop)
    CARD32 start, stop;
    xTimecoord *buff;
{
    return 0;
...
/*ARGSUSED*/
static void
macmachMouseDoneEvents (pMouse,final)
    DevicePtr	  pMouse;
    Bool	  final;
{
    PtrPrivPtr	  pPriv;
    macmachMsPrivPtr  pmacmachPriv;
    int x , y;

    pPriv = (PtrPrivPtr) pMouse->devicePrivate;
    pmacmachPriv = (macmachMsPrivPtr) pPriv->devPrivate;

    if (pmacmachPriv->mouseMoved) {
	x = pPriv->x; y = pPriv->y;
	pPriv->x = 0; pPriv->y = 0;
	pmacmachPriv->mouseMoved = FALSE;
	miPointerDeltaCursor (screenInfo.screens[0], x, y, TRUE);
    }
}

void
macmachMouseProcessEvent(pMouse,me)
    DeviceRec *pMouse;
    register unsigned char *me;
{   
    xEvent		xE;
    register PtrPrivPtr	pPriv;	/* Private data for pointer */
    register macmachMsPrivPtr pmacmachPriv; /* Private data for mouse */
...
/*ARGSUSED*/
static Bool
macmachCursorOffScreen (pScreen, x, y)
    ScreenPtr   *pScreen;
    int         *x, *y;
{
  int index;
...
static void
macmachCrossScreen (pScreen, entering)
    ScreenPtr   pScreen;
    Bool        entering;
{
}
