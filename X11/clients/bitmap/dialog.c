...
#ifndef lint
static char *rcsid_dialog_c = "$XConsortium: dialog.c,v 1.6 89/12/10 15:54:07 rws Exp $";
#endif
...
int dialog (w, font,
  ...
  while (1) {
    ...
    if (event.xany.window == data.w) {
      static ProcessDialogWindowEvent();
    ...
    if (command) {
       static int ProcessCommandEvent();
