...
{"-blt",	"*bltScroll",	XrmoptionNoArg,		(caddr_t) "on"},
{"+blt",	"*bltScroll",	XrmoptionNoArg,		(caddr_t) "off"},
...
char **argv;
{
	...
	*/
...
#if defined att || defined macII || !defined BAUD_0
#ifdef B38400
    	d_tio.c_cflag |= B38400;
#else	/* !B38400 */
#ifdef B19200
    	d_tio.c_cflag |= B19200;
#else	/* !B19200 */
    	d_tio.c_cflag |= B9600;
#endif	/* !B19200 */
#endif	/* !B38400 */
#endif	/* defined att || defined macII || !defined BAUD_0 */
...
#endif /* defined(macII) || defined(att) */
#endif /* CSWTCH */
#endif	/* VSWTCH */
#ifdef VEOL2
	...
	/* now, try to inherit tty settings */
	{
        ...
        d_ltc.t_werasc = 0;
        d_ltc.t_lnextc = 0;
#else /* !defined macII && !defined att */
#ifdef CRPRNT
        d_ltc.t_rprntc = CRPRNT;	/* reserved...*/
#else
	...
	for (argc--, argv++ ; argc > 0 ; argc--, argv++) {
		...
		Console = TRUE;
#else
		(void) fprintf(stderr, "%s: console function unavailable\n",
			       xterm_name);
#endif	/* TIOCCONS */
		continue;
	     case 'S':
 ...
 */
{
...
#if !defined USE_SYSV_ENVVARS && defined USE_TERMCAP
	char termcap [1024];
	char newtc [1024];
#endif /* !defined USE_SYSV_ENVVARS && defined USE_TERMCAP */
	...
	if (!am_slave) {
		 ...
		 */
#ifdef att
		/* If we set our process group here any later
		 * attempt to set out session will fail
		 */
		(void) setpgrp();
#endif /* att */
		while (1) {
...
#ifdef sgi
#ifdef __KERNEL_BUSTED__TIOCNOTTY_MAKES_PROC_GROUP_LEADER__
				BSDsetpgrp( 0, 0 ) ;
#endif /* __KERNEL_BUSTED__TIOCNOTTY_MAKES_PROC_GROUP_LEADER__ */
#endif /* sgi */
				close (tty);
			}
#ifdef att
			/* If this "unix" has TIOCNOTTY, we may need
			 * to recreate our process group in order to
			 * open "/dev/tty" !
			 */
			(void) setpgrp();
#endif /* att */
		...
		/* Now that we have our new tty, let's make
		** ourselves the process (and session) group(s)
		** leader.
		** NOTE: to become the session group leader you
		**	 MUST NOT YET be a process group leader !
		**	 ( sorry att )
		*/
...
#endif
#ifdef HAS_POSIX_SESSIONS
		if ( setsid() == pgrp )
			/* NOTE: we no longer have any controlling tty ! */
			(void) close( open( ttydev, O_WRONLY, 0 ) ) ;
#endif
#ifdef TIOCSCTTY
		(void) ioctl( 0, TIOCSCTTY, (char *) &pgrp ) ;
#endif
#ifdef USE_SYSV_PGRP
		(void) setpgrp() ;
#else
...
#else
		envnew = (char **) calloc ((unsigned) i + (3 + 1), sizeof(char *));
#endif /* USE_TERMCAP */
...
#ifdef NTTYDISC
#ifdef USE_SYSV_TERMIO
		/* If we're SYSV, the line discipline is probably already
		 * correct for most shells, BUT if NTTYDISC exists it is
		 * likely to be the best choice for a JOBCONTROL knowledgable
		 * shell like "csh" or "ksh"
		 */
		if ( XStrCmp("csh", shname + strlen(shname) - 3) == 0
		  || XStrCmp("ksh", shname + strlen(shname) - 3) == 0 ) {
		    ldisc = NTTYDISC ;
