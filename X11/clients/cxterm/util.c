...
#ifndef lint
static char rcs_id[] = "$XConsortium: util.c,v 1.20 90/03/14 17:04:59 jim Exp $";
#endif	/* lint */
...
register TScreen *screen;
{
	...
	if (screen->bltscroll) {
		...
		if (refreshheight + scrollheight > 0)
			ScrnRefresh(screen,
				    refreshtop < scrolltop ? refreshtop : scrolltop,
				    0, scrollheight + refreshheight,
...
register int amount;
{
	...
	if(refreshheight > 0 && screen->bltscroll) {
		XClearArea (
		   screen->display,
		   TextWindow(screen),
		   (int) screen->border + screen->scrollbar,
...
register TScreen *screen;
{
	...
	bzero (screen->buf [4 * screen->cur_row], (screen->cur_col + 1));
	bzero (screen->buf [4 * screen->cur_row + 1], (screen->cur_col + 1));
