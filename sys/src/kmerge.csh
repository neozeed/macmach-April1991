...
set def=no
echo ""
echo -n "Has this kernel been built and tested for the IBMRT, SUN3 and VAX?  [$def]  "
set ans="$<"
if ($ans:q == "") set ans=$def:q
if ($ans:q !~ [Yy] && $ans:q !~ [Yy][Ee][Ss]) then
...
if (! $?release) then
    set bboard=(mach-kernellog)
else
    set bboard=(unix-changelog)
endif

set def=no
echo -n "Has a post already been completed to the $bboard bboard?  [$def]  "
set ans="$<"
if ($ans:q == "") set ans=$def:q
if ($ans:q !~ [Yy] && $ans:q !~ [Yy][Ee][Ss]) then
    echo ""
    echo "[ please draft a post for the $bboard bulletin board ]"
...
[
  ...
  p[ost]  - post the log message to '$bboard'
]
!
	    breaksw
