 ...
 * Revision 2.22  89/12/22  16:26:09  rpd
 * 	Updated to new SERVER_LOOP interface.
 * 	[89/11/24  23:02:40  rpd]
 * 
 * Revision 2.21  89/12/22  15:52:07  rpd
 * 	PAGE_WAKEUP --> PAGE_WAKEUP_DONE.  See vm/vm_page.h.
 * 	[89/12/13            dlb]
 * 
 * Revision 2.20  89/10/10  10:48:41  mwyoung
 * 	Use "private" instead of "fictitious" page attribute.
 * 	[89/04/22            mwyoung]
 * 
	...
	vm_prot_t desired_access;
{
	...
	if (d->pages == 0) {
	    (void) memory_object_data_error(memory_control_port,
					    offset,
					    length, 0);
	    return (KERN_SUCCESS);
	}
...

	{
	    ...
	    if (object_copyin(device_pager_task,
			      memory_control_port,
			      MSG_TYPE_PORT, FALSE,
			      (kern_obj_t *) &pager_port)) {
	...
	pg_cur = &d->pages[atop(offset)];
	offset += d->offset;
	for (num_pages = atop(length);
	     num_pages > 0;
	     num_pages--, pg_cur++, offset += PAGE_SIZE) {
		vm_offset_t	addr;
		vm_page_t	old_page;

		/*
		 *	Insert a page with the appropriate physical
		 *	address.  [In the event that a thread using this data
		...
		if ((old_page =
		     vm_page_lookup(object, offset)) != VM_PAGE_NULL) {
		    	if (old_page == pg_cur)
			    continue;
		...
		/*
		 *	Wire down the kernel memory page
		 *	that contains this vm_page structure.
		 */
	...
	vm_size_t		pager_page_size;
{
	...
	d->pages = (vm_page_t) kmem_alloc_pageable(kernel_map, d->pages_size);

	if (d->pages == 0)
	    d->pages_size = 0;
	else
	    (void) memory_object_set_attributes(pager_request,
						TRUE, FALSE,
	...
	if (d == DEV_PAGER_NULL) {
		...
		if (d->pages)
		    kmem_free(kernel_map,
			      (vm_offset_t) d->pages, d->pages_size);
...
void		device_pager()
{
	...
	SERVER_LOOP(device_pager_enabled_set, TRUE);
}
