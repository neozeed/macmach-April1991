 ...
 * HISTORY
 * $Log:	ufs_dir.c,v $
 * Revision 2.9  90/02/09  12:34:01  berman
 ...
 * 	[90/02/09            berman]
 * 
 * Revision 2.8  89/09/09  16:23:50  rvb
 * 	Add VFMT case for dirmakeinode() for badsect.
 * 	[89/08/29            rvb]
 * 
 * Revision 2.7  89/09/05  20:45:21  jsb
 * 	Added code for append-only directories (via sticky bit).
 * 	[89/09/05  15:10:53  jsb]
 * 
 * Revision 2.6  89/08/02  08:10:28  jsb
 * 	Eliminate MACH conditionals.
 * 	[89/07/31  15:50:24  jsb]
 * 
 * Revision 2.5  89/04/22  15:33:20  gm0w
 * 	Added new NBC code from rfr.  Changed to use new inode macros for
 * 	manipulation of inodes.
 * 	[89/04/14            gm0w]
 * 
 * Revision 2.4  89/03/09  22:43:28  rpd
 * 	More cleanup.
 * 
 * Revision 2.3  89/02/26  11:12:08  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.2  89/01/18  01:23:14  jsb
 * 	Include file references;
 * 	MACH: add hack in dirlook so that it ignores the protection of a
 * 	directory that is currently mounted-on. This is necessary because of a
 * 	bug in the design of vfs_lookup, in which it uncovers a mounted-on
 * 	vnode and then checks access, as opposed to namei (which gets this
 * 	right) which checks access first.
 * 	[89/01/17  15:43:48  jsb]
 * 
 */
/* @(#)ufs_dir.c	1.5 87/09/22 3.2/4.3NFSSRC */
	...
	register struct inode **ipp;
{
	...
	/*
	 *  This is necessary due to bug in design of vfs_lookup/VOP_LOOKUP
	 *  interaction, which ends up (here) checking protection of mounted-
	 *  upon directory instead of that of the root of the mounted
	 *  filesystem (which is what namei does, and what should be done).
	 */
	if (! ITOV(dp)->v_vfsmountedhere && (error = iaccess(dp, IEXEC)))
		return (error);
	...
	struct inode **ipp;		/* return entered inode (locked) here */
{
	...
	if (tip) {
		 ...
		 */
		if (op == DE_RENAME && (sdp->i_mode & ISVTX) && u.u_uid != 0 &&
			u.u_uid != sdp->i_uid && u.u_uid != sip->i_uid) {
			error = EPERM;
			goto out;
		}
		if (op == DE_CREATE) {
...
#if	MACH_NBC
	if (sip != NULL && sip->i_nlink == 0)
		inode_uncache(ITOV(sip));
#endif	MACH_NBC
	iunlock(tdp);
	return (error);
}
	...
	register struct inode *npdp;	/* new parent directory */
{
	...
	if (opdp->i_nlink != 0) {
		opdp->i_nlink--;
		imark(opdp, ICHG);
		iupdat(opdp, &time, &time, 1);
	}
#if	MACH_NBC
	if (opdp->i_nlink == 0)
		inode_uncache(ITOV(opdp));
#endif	MACH_NBC
	iunlock(opdp);
	return (0);
bad:
	...
	register struct vattr *vap;
{
	...
	if (type == VFMT) {
		ip->i_rdev = vap->va_fsid;
		vap->va_fsid = -1;		/* initial state after vattr_null() */
	}
	...
	register struct inode *dp;		/* parent directory */
{
	...
	fs = ip->i_fs;
	bn = fsbtodb(fs,
	    bmap(ip, (daddr_t)0, B_WRITE, DIRBLKSIZ, 0));
	if (bn <= 0 || u.u_error) {
		if (u.u_error == 0)
			u.u_error = ENOSPC;
		return (u.u_error);
	...
	bp = getblk(ip->i_devp, bn, (int)fs->fs_fsize);
...
#if	MACH_NBC
		/*
		 * Check to see if we are deleting this file. 
		 * If we are, then we must remove it from the cache.
		 */
		if (ip->i_nlink == 0)
