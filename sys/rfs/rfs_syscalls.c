 ...
 * Revision 2.16  89/10/10  10:58:36  mwyoung
	...
	int	nc;
{
	...
	if(nc < MAX_FASTLINK_SIZE && create_fastlinks) {
		u.u_error = copyin(target, ip->i_symlink, nc);
		ip->i_size = nc;
		ip->i_icflags |= IC_FASTLINK;
		ip->i_flag |= IACC|IUPD|ICHG;
	} else
#endif	MACH_FASTLINK
	u.u_error = rdwri(UIO_WRITE, ip, target, nc, (off_t)0, 0,
	    (int *)0);
