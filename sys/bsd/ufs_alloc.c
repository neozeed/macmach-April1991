 ...
 * HISTORY
 * $Log:	ufs_alloc.c,v $
 * Revision 2.10  90/02/09  12:33:33  berman
 * 	Added checks on u_VOPuid field in alloc() and realloccg() 
 * 	to allow afsd to write into the file system reserve.
 * 	[90/02/09            berman]
 * 
 * Revision 2.9  89/10/10  10:47:26  mwyoung
 * 	Make fast symbolic links an architecture-independent option.
 * 	[89/05/12  16:00:46  mwyoung]
 * 
 * Revision 2.8  89/04/22  15:21:45  gm0w
 * 	Changed to use new inode macros for manipulation of inodes.
 * 	[89/04/14            gm0w]
 * 
 * Revision 2.7  89/02/26  11:28:21  gm0w
 * 	Changes for cleanup.
 * 
 * Revision 2.6  89/02/09  04:34:04  mwyoung
 * Code cleanup cataclysm.
 * 
 * Revision 2.5  89/01/18  00:42:44  jsb
 * 	Vnode support.
 * 	[89/01/16            jsb]
 * 
	...
	int size;
{
	...
	if ((unsigned)size > fs->fs_bsize || fragoff(fs, size) != 0) {
		printf("dev = 0x%x, bsize = %d, size = %d, fs = %s\n",
		    INODE_DEV(ip), fs->fs_bsize, size, fs->fs_fsmnt);
		panic("alloc: bad size");
	}
	if (size == fs->fs_bsize && fs->fs_cstotal.cs_nbfree == 0)
		goto nospace;
	if (u.u_uid != 0 && u.u_VOPuid != 0
	...
	CHGUPD_INODE(ip);
	bp = getblk(INODE_DEV(ip), fsbtodb(fs, bno), size);
	clrbuf(bp);
	return (bp);
nospace:
	...
	int osize, nsize;
{
	...
	if ((unsigned)osize > fs->fs_bsize || fragoff(fs, osize) != 0 ||
		...
		printf("dev = 0x%x, bsize = %d, osize = %d, nsize = %d, fs = %s\n",
		    INODE_DEV(ip), fs->fs_bsize, osize, nsize, fs->fs_fsmnt);
	...
	if (bprev == 0) {
		printf("dev = 0x%x, bsize = %d, bprev = %d, fs = %s\n",
		    INODE_DEV(ip), fs->fs_bsize, bprev, fs->fs_fsmnt);
	...
	if (bno != 0) {
		do {
			bp = bread(INODE_DEV(ip), fsbtodb(fs, bno), osize);
	...
	bno = (daddr_t)hashalloc(ip, cg, (long)bpref, request,
		(u_long (*)())alloccg);
	if (bno > 0) {
		obp = bread(INODE_DEV(ip), fsbtodb(fs, bprev), osize);
		if (obp->b_flags & B_ERROR) {
			brelse(obp);
			return (NULL);
		}
		bn = fsbtodb(fs, bno);
		bp = getblk(INODE_DEV(ip), bn, nsize);
	...
	int mode;
{
	...
	if (fs->fs_cstotal.cs_nifree == 0)
		goto noinodes;
#if	QUOTA
	u.u_error = chkiq(QUOTA_DEV(pip), (struct inode *)NULL, u.u_uid, 0);
	if (u.u_error)
		return (NULL);
#endif	QUOTA
	...
	int osize, nsize;
{
	...
	bp = bread(INODE_DEV(ip), fsbtodb(fs, cgtod(fs, cg)), (int)fs->fs_cgsize);
	cgp = bp->b_un.b_cg;
	...
	if (size == fs->fs_bsize) {
		...
		for (i = 0; i < frags; i++) {
			if (isset(cgp->cg_free, bno + i)) {
				printf("dev = 0x%x, block = %d, fs = %s\n",
				    INODE_DEV(ip), bno + i, fs->fs_fsmnt);
