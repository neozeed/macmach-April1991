 ...
 * HISTORY
 * $Log:	kern_exit.c,v $
 * Revision 2.19.1.1  90/09/07  00:52:29  bohman
 * 	Final cleanup for mac2.
 * 	[90/09/07            bohman]
 * 
 * Revision 2.19  90/08/30  10:58:38  bohman
 * 	Changes for mac2.
 * 	[90/08/28            bohman]
 * 
 * Revision 2.18  90/08/08  17:15:57  mrt
 ...
 * 
 * Revision 2.17  90/04/28  00:02:37  jjc
	...
	int rv;
{
	 ...
	 */
	utvp = &u.u_ru.ru_utime;
	utvp->tv_sec = 0;
	utvp->tv_usec = 0;

	stvp = &u.u_ru.ru_stime;
	stvp->tv_sec = 0;
	stvp->tv_usec = 0;
...
#if	defined(multimax) || defined(ibmrt) || defined(balance) || defined(vax) || defined(sun3) || defined(i386) || defined(mac2)
...
wait()
{
	...
	u.u_error = wait3(options=fuword(u.u_ar0[R1] + 4), &ru, -1);
#else	multimax

#if	defined(mac2)
	if ((current_thread_pcb()->pcb_frame->f_normal.f_sr & PSL_ALLCC) != PSL_ALLCC)
#endif	defined(mac2)
#ifdef	ibmrt
...
#endif	i386
	{
		u.u_error = wait3(0, (struct rusage_dev *)0, -1);
		return;
	}
#ifdef	ibmrt
	rup = (struct rusage_dev *)u.u_ar0[R4];	 /* as per C linkage */
	u.u_error = wait3(options=u.u_ar0[R3], &ru, -1);     /* as per C linkage */
#endif	ibmrt
#ifdef	balance
	rup = (struct rusage_dev *)u.u_ar0[R2];
	u.u_error = wait3(options=u.u_ar0[R1], &ru, -1);
#endif	balance
#ifdef	vax
	rup = (struct rusage_dev *)u.u_ar0[R1];
	u.u_error = wait3(options=u.u_ar0[R0], &ru, -1);
#endif	vax
#if	defined(mac2)
	rup = (struct rusage_dev *)u.u_ar0[R1];
	u.u_error = wait3(options=u.u_ar0[R0], &ru, -1);
	...
	u.u_error = wait3(options=u.u_ar0[ECX], &ru, -1);
#endif	i386
#endif	multimax
...
#else	defined(multimax) || defined(ibmrt) || defined(balance) || defined(vax) || defined(sun3) || defined(i386) || defined(mac2)
...
#endif	defined(multimax) || defined(ibmrt) || defined(balance) || defined(vax) || defined(sun3) || defined(i386) || defined(mac2)
		...
		if (p->task == TASK_NULL)
		{
			...
			if((pid != -1)) {
				if((pid > 0) && (pid != p->p_pid))
					continue;
				if((pid == 0) && (q->p_pgrp != p->p_pgrp))
					continue;
				if((pid < 0) && (-pid != p->p_pgrp))
					continue;
			}
#endif
...
waitpid()
{
	...
	u.u_error = wait3(uap->options, (struct rusage *)0, uap->pid);
}
