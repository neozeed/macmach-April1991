 ...
 * 
 *	Put the calls to binit and bhinit back in for those
 *	architectures that do not have disklabeling yet
 * 	[90/05/17  17:46:24  mrt]
 * 
 * Revision 2.28  90/02/09  12:33:26  berman
 * 	Added initialization of new u_VOPuid field in main(). 
 * 	from jsb
 * 	[90/02/09            berman]
 * 
 * Revision 2.27  89/12/22  16:25:46  rpd
 * 	Size the inode_pager's map according to how many threads it has.
 * 	[89/11/28  21:30:21  rpd]
 * 
 * Revision 2.26  89/10/11  13:34:14  dlb
 * 	Add action and sched threads (action is multiprocessor only).
 * 	Add kernel monitor support, remove timestamp support.
 * 	Convert to processor logic.
 * 	Remove obsolete service_timers() kickoff
 * 
 * Revision 2.25  89/10/10  10:46:03  mwyoung
 * 	Remove lint: declare device_init().
 * 	[89/08/07            mwyoung]
 * 	Indicate that allocations in the kernel_pageable_map
...
main()
{
	...
	u.u_modes   = umodes;
	u.u_maxuprc = maxuprc;
	u.u_VOPuid  = 1; /* not root, which is all that matters */
...
#include <sl.h>
	...
	pqinit();
	ihinit();
#if !defined(vax) && !defined(ibmrt) && !defined(sun3) && !defined(sun4)
	bhinit();  /* done in startup for machines with disklabel code */
#endif
	NAME_CACHE_INIT();
	/*
...
#if !defined(vax) && !defined(ibmrt) && !defined(sun3) && !defined(sun4)
	binit();  /* done in startup for machines with disklabel code */
#endif
#ifdef	GPROF
	kmstartup();
#endif	GPROF
...
/* set up the root file system */
	...
	/*inode_pager*/ {
	extern int	inode_pager_num_sets;
	thread_t	inode_th;
	vm_offset_t	min, max;
	...
	inode_th->task->map =
		kmem_suballoc(kernel_map, &min, &max,
			      64 * inode_pager_num_sets * PAGE_SIZE,
			      FALSE);
	...
	inode_th->max_priority = BASEPRI_SYSTEM;
	inode_th->priority = BASEPRI_SYSTEM;
	inode_th->sched_pri = BASEPRI_SYSTEM;
	...
	device_th->max_priority = BASEPRI_SYSTEM;
	device_th->priority = BASEPRI_SYSTEM;
	device_th->sched_pri = BASEPRI_SYSTEM;
	...
	device_io_th->max_priority = BASEPRI_SYSTEM;
	device_io_th->priority = BASEPRI_SYSTEM;
	device_io_th->sched_pri = BASEPRI_SYSTEM;
	...
	{ /*mac_kernel_server*/
	    thread_t	thread;
	    extern void	macserver();

	    thread = newproc(0);
	    thread->ipc_kernel = TRUE;
	    thread->task->kernel_vm_space = TRUE;
	    macserver_init(thread->task, th->task);
	    ...
	    thread_start(thread, macserver, THREAD_SYSTEMMODE);
	...
	(void) thread_swappable(kernel_thread(first_task,
					      vm_pageout),
				FALSE);
	...
	(void) thread_swappable(kernel_thread(first_task,
				...
				FALSE);
#if	NCPUS > 1
	(void) kernel_thread(first_task, action_thread);
#endif	NCPUS > 1
	(void) kernel_thread(first_task, sched_thread);
...
/*
 * Sync the disks when shutting down
 */
