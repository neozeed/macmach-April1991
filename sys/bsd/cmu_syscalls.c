 ...
 * 	[90/03/26            dlb]
 * 
 * Revision 2.16  90/01/11  14:49:41  berman
 * 	Added unsys() for general use when the syscall is unsupported.
 * 	[90/01/11            berman]
 * 
 * Revision 2.15  89/10/10  10:45:43  mwyoung
 * 	Use new vm_map_copy technology.
 * 	[89/06/26  18:53:37  mwyoung]
 * 
		...
		case TBL_UAREA:
		   {
			...
			fake = (struct user *)
				kmem_alloc_pageable(kernel_pageable_map,
			...
			dealloc_size = round_page(sizeof(struct user));
			break;
		   }
...
#ifdef	PSDEBUG
			if(psdebug) {
				printf("getting %d bytes of args\n", arg_size);
				printf("arg_addr = %x, copy_size = %d\n",
					arg_addr, copy_size);
			}
#endif
...
#ifdef	PSDEBUG
			if(psdebug)
				printf("got %d at %x; copy...", copy_size,
					copy_start);
#endif
...
#ifdef	PSDEBUG
			if(psdebug)
				printf("searching: ip %x, lastp %x, data %x, size %d\n",
					ip, lastp, data, size);
#endif
...
#ifdef	PSDEBUG
				if(psdebug && uap->id == TBL_ARGUMENTS)
					printf("copyout(%x, %x, %d)\n",
						data, uap->addr, size);
#endif
...
#ifdef	PSDEBUG
			if(psdebug && uap->id == TBL_ARGUMENTS)
				printf("DEALLOC %d at %x\n",
					dealloc_size, dealloc_start);
#endif
...
/*
 * Following is useful for unimplemented vendor specific syscalls
 */
unsys() {
	u.u_error = EINVAL;
}
