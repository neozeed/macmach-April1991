...
read_text:
...
#ifdef	i386
		/* (af)  For how long should we tolerate this ? */
		/*
		 ...
		 */
		suiword(0, 0);
		vm_map_protect(my_map, 0, round_page(0 + sizeof(int)),
...
#if	BSD_A_OUT
#if	defined(vax) || defined(sun) || defined(ibmrt) || defined(i386) || defined(mac2)
...
#endif	vax
#if	defined(sun) || defined(ibmrt) || defined(i386) || defined(mac2)
	text_size = round_page(real_text_size);
#endif	defined(sun) || defined(ibmrt) || defined(i386) || defined(mac2)
...
#define USRTEXT	0x10000
	...
	if (pagi == 0) {
		...
		if ((u.u_error == 0) && (real_text_size > 0)) {
			...
			if (u.u_error == 0 && trunc_page(text_size) > 0) {
				(void) vm_protect(my_map,
					 text_start,
...
#define PROT (allow_0_reference ? VM_PROT_READ : VM_PROT_NONE)
	addr = 0;
	if (vm_map(my_map, &addr, USRTEXT, 0, FALSE,
		MEMORY_OBJECT_NULL, (vm_offset_t) 0, FALSE,
		VM_PROT_NONE, VM_PROT_NONE, VM_INHERIT_SHARE)
	     != KERN_SUCCESS) {
		uprintf("Cannot reserve user page 0.\n");
