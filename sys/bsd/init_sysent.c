 ...
 * HISTORY
 * $Log:	init_sysent.c,v $
 * Revision 2.22  90/08/08  17:15:47  mrt
 * 	Added more Ultrix4.0 compat entries for mips.
 * 	[90/05/20  13:46:44  af]
 * 
 * Revision 2.21  90/04/28  00:02:23  jjc
 ...
 * 	[90/03/01            jjc]
 * 
 * Revision 2.20  90/01/11  11:45:57  berman
 ...
 * 	[90/01/04            berman]
 * 
 * Revision 2.19  89/10/11  13:35:09  dlb
 * 	Add multimax branch, including two multimax-specific syscalls.
 * 
...
#ifdef	multimax
/*
 *	Multimax-specific call to map free running counter.
 */
int	mapfrcounter(), old_mapfrcounter();
#endif	multimax
#ifdef	mips
...
#if	defined(multimax) || defined(ibmrt) || defined(balance) || defined(vax) || defined(sun3) || defined(i386)
	syss(wait,0),			/*  84 = wait */
#else	defined(multimax) || defined(ibmrt) || defined(balance) || defined(vax) || defined(sun3) || defined(i386)
	syss(wait,3),			/*  84 = wait */
#endif	defined(multimax) || defined(ibmrt) || defined(balance) || defined(vax) || defined(sun3) || defined(i386)
...
#ifdef	multimax
#undef	GENERIC
	syss(old_mapfrcounter,2),	/* 151 = old_mapfrcounter */
	syss(mapfrcounter,2),		/* 152 = mapfrcounter */
	syss(nosys,0),			/* 153 */
	syss(nosys,0),			/* 154 */
	syss_nfs_svc(),			/* 155 = nfs_svc */
	syss_getdirentries(),		/* 156 = getdirentries */
	...
	syss(setsid,0),			/* 188 = setsid */
	syss(waitpid,3),		/* 189 = waitpid */
#else	PMAX
#ifdef	ibmrt
	...
	syss(nosys,0),			/* 250 */
	syss(nosys,0),			/* 251 */
	syss(nullsys,0),		/* 252 audcntl */
	syss(nullsys,0),		/* 253 audgen */
	syss(unsys,0),			/* 254 startcpu */
	syss(unsys,0),			/* 255 stopcpu */
	...
	syss(nosys,0),			/* 254 */
	syss(nosys,0),			/* 255 */
	syss(unsys,6),			/* 256 = getsysinfo */
	syss(unsys,5),			/* 257 = setsysinfo */
#endif	vax
};
