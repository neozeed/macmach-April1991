 ...
 * we cannot use the standard telldir since we are
 * not updating dirp->dd_entno.
 */
static long telldir(dirp)
	DIR *dirp;
{
	extern long lseek();
...
/*
 * open a directory
 * we can not use the standard opendir since it will
 * only open REAL directories, not the fake ones used here.
 */
