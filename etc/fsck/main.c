...
#ifdef mac2
	pwarn("%d Unix files, ", n_files - nViceFiles);
	if (nViceFiles)
		printf("%d Vice files, ", nViceFiles);
	printf("%d blocks used, %d free ",
	     n_blks, n_ffree + sblock.fs_frag * n_bfree);
#else
...
#ifdef mac2
	if (debug && (n_files -= (sblock.fs_ncg*sblock.fs_ipg) - ROOTINO - sblock.fs_cstotal.cs_nifree))
#else
...
#ifdef mac2
	if (nViceFiles) {
	    /* there should be code here to do this... */
	    printf("\n***** VICE FILE SYSTEM WAS MODIFIED *****\n");
	    printf("\n***** RUN FULL SALVAGE ON PARTITION *****\n");
	    exit(8);
	}
#endif
}
