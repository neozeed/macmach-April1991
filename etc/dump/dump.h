...
#ifdef mac2
/* BUG FIX: if /etc/dumpdates is not found then idate will be NULL
 *          this means that we would try to dereference a NULL pointer
 *	    on the mac (and sun) this is fatal
 */
#define	ITITERATE(i, ip) \
	if (idatev) \
		for (i = 0,ip = idatev[0]; i < nidates; i++, ip = idatev[i])
#else mac2
