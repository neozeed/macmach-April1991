	...
	char **argv;
{
	...
	if (header) {
		 ...
		 */
		lseek(kmem, (long)nl[X_BOOTTIME].n_value + kbase, 0);
		...
		if (use_mach_factor && nl[X_MACH_FACTOR].n_value != 0) {
		  printf(",  Mach factor:");
		  lseek(kmem, (long)nl[X_MACH_FACTOR].n_value + kbase, 0);
		  read(kmem, machfactor, sizeof(machfactor));
		  for (i = 0; i < 3; i++)
		    avenrun[i] = (float) machfactor[i] / (float) LSCALE;
		}
		else {
		  printf(",  load average:");
		  if (table(TBL_LOADAVG,
		      0,
		      &tbl_loadavg,
		      1,
		      sizeof(tbl_loadavg)) == 1) for (i = 0; i < 3; i++)
		    if (!tbl_loadavg.tl_lscale) avenrun[i] = tbl_loadavg.tl_un.tl_davenrun[i];
		    else avenrun[i] = ((double)(tbl_loadavg.tl_un.tl_lavenrun[i])) /
		      ((double)(tbl_loadavg.tl_lscale));
		  else for (i = 0; i < 3; i++) avenrun[i] = 0.0;
		}
#else	MACH
		printf(",  load average:");
		lseek(kmem, (long)nl[X_AVENRUN].n_value + kbase, 0);
...
readpr()
{
	...
	Usrptma = (struct pte *) (nl[X_USRPTMA].n_value + kbase);
	usrpt = (struct pte *) (nl[X_USRPT].n_value + kbase);
	 ...
	 */
	lseek(kmem, (long)nl[X_SWAPDEV].n_value + kbase, 0);
	read(kmem, &nl[X_SWAPDEV].n_value, sizeof(nl[X_SWAPDEV].n_value));
	/*
	 * Find base of and parameters of swap
	 */
	lseek(kmem, (long)nl[X_NSWAP].n_value + kbase, 0);
	read(kmem, &nswap, sizeof(nswap));
	lseek(kmem, (long)nl[X_DMMIN].n_value + kbase, 0);
	read(kmem, &dmmin, sizeof(dmmin));
	lseek(kmem, (long)nl[X_DMMAX].n_value + kbase, 0);
	 ...
	 * Locate proc table
	 */
	lseek(kmem, (long)nl[X_NPROC].n_value + kbase, 0);
	read(kmem, &nproc, sizeof(nproc));
	pr = (struct pr *)calloc(nproc, sizeof (struct pr));
	np = 0;
	lseek(kmem, (long)nl[X_PROC].n_value + kbase, 0);
