...
#ifdef mac2
    asm("clrl sp@-; jbsr _sbrk; movel d0,minbrk");
#else
    minbrk = sbrk(0);
#endif
    tos[0].link = 0;
...
mcount()
{
#ifdef mac2
	char				*selfpc;	/* a6@(-4) */
	unsigned short			*frompcindex;	/* a6@(-8) */
#else
	 ...
	 */
#ifdef mac2
	asm("	movl a6@(4),a6@(-4)");	/* return address for mcount */
	asm("	movl a6@,a0");		/* move back one stack frame */
	asm("	movl a0@(4),a6@(-8)");	/* return address for mcount's caller */
#else
...
#ifdef mac2
/* change _mcount to mcount */
asm(".set mcount, _mcount; .globl mcount");
#endif
