...
#ifdef CMU
  /* If the -MD option was used, output the deps to "<input file>.d". */
...
do { ip = &instack[indepth];		\
  ...
  while (1) {
	...
	*obp++ = '*';
	...
	} else assert (0);	/* Newline followed by something random?  */
	break;
      }
