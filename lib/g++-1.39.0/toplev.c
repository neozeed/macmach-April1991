...
/* Change by Bryan Boreham, Kewill, Sun Jul 23 14:41:26 1989.
   Added some stuff for unexec-ing a part-done compilation. */

int been_here_before = 0;	/* Is this an unexec'd program? */
int just_done_unexec = 0;	/* Is this the process that *was* unexec'd?  */
char *dump_source_name;		/* Keep argv[0] for dumping. */

/* @@ Should there be a filename length limit?  */
char previous_asm_file_name[128]; /* asm_file_name from the first run. */
...
/* 1 => write gdb debugging output (using symout.c).  -g0
   2 => write dbx debugging output (using dbxout.c).  -G0  */
...
#ifdef G_SWITCH
/* Value of the -G nn switch if the machine needs it.  */
...
#ifdef FIELD_XREF
  int field_xref = 0;		/* cross referencing flag	*/
#endif

     ...
     tree decl;
{
  if (! quiet_flag)
	...
	fprintf (stderr, " %s", (*decl_printable_name) (decl));
      fflush (stderr);
     ...
     int v;
{
  ...
  if (DECL_NAME (decl))
    fprintf (stderr, s, (*decl_printable_name) (decl), v);
  else
     ...
     char *name;
{
  ...
  if (dump_base_name == 0 || been_here_before)
  ...
  /* Initialize data in various passes,  
     checking if this is an unexec'd program - bryan@kewill.uucp.  */
  ...
  if (!been_here_before)
    ...
    {
      extern void reinit_lex_for_unexec ();
...
#ifdef FIELD_XREF
  if (field_xref) FIELD_xref_begin(input_filename);
#endif
  ...
  if (DECL_SAVED_INSNS (decl) == 0)
	      ...
	      || TREE_EXTERNAL (decl)))
	{
	 ...
	 so that its compilation will not affect what others get.  */
      if (TREE_INLINE (decl))
	TIMEVAR (integration_time, save_for_inline (decl));
    }
     ...
     char *path, *name;
{
  ...
  data = (char *)((int)&my_edata&~(getpagesize()-1));
  if ((int)mmap (data, file_size, PROT_READ|PROT_WRITE,
		 MAP_PRIVATE|MAP_FIXED, file, 0) < 0)
    {
      perror (name);
      exit (-1);
    }
     ...
     char *name;
{
  ...
  brk((char*)((int)&my_edata&~(getpagesize()-1)) + file_size);
...
#if defined(ASM_OUTPUT_SOURCE_FILENAME) && defined(ASM_OUTPUT_SOURCE_LINE)
	  else if (!strcmp (str, "gline"))
	    write_symbols = LINENO_DEBUG;
#endif
#if 0
...
#ifdef FIELD_XREF
  FIELD_xref_end(errorcount+sorrycount);
#endif
